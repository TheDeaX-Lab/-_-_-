...
begin
  ...
  // Если есть неудовлетворительные оценки
  if (mark_counts[0] > 0) or (mark_counts[1] > 0) then
    // Стипендии не будет
    factor := 0
...
begin
  ...
  begin
    if student.marks[i].discipline_id = student.marks[i].discipline_id then begin
